...
namespace InventarioWebApp
{
    ...
    {
        public int AgregarCierre(int idUsuario, int idSucursal, String RutEmpresa, int computador, int devolucion, int ingresos, DataTable egresos, DataTable dtIngresos, DataTable ventaMenor, DataTable creditos, ArrayList detalleDinero)
        {
            ...
            arrDoc=daoDoc.seleccionaDocumento(2,RutEmpresa,0,false,2,0,0);

            foreach(Documentos objDoc in  arrDoc){
                daoDoc.ModificarEstadoDocumento(objDoc.idDocumento, objDoc.estadoDocumento, 0, 1);
            }
